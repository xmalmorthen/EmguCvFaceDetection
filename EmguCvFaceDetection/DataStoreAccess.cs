    ...
    class DataStoreAccess : IDataStoreAccess
    {
        ...
        public String SaveFace(string username, Byte[] faceBlob)
        {
            try
            {
                ...
                var insertQuery = "INSERT INTO caImages(username, faceSample, userId) VALUES(@username,@faceSample,@userId)";
        ...
        public List<Face> CallFaces(string username)
        {
            ...
            {
                _sqLiteConnection.Open();
                var query = username.ToLower().Equals("ALL_USERS".ToLower()) ? "SELECT * FROM caImages" : "SELECT * FROM caImages WHERE username=@username";
        ...
        public int GetUserId(string username)
        {
            ...
            {
                _sqLiteConnection.Open();
                var selectQuery = "SELECT userId FROM caImages WHERE username=@username LIMIT 1";
        ...
        public string GetUsername(int userId)
        {
            ...
            {
                _sqLiteConnection.Open();
                var selectQuery = "SELECT username FROM caImages WHERE userId=@userId LIMIT 1";
                var cmd = new SQLiteCommand(selectQuery, _sqLiteConnection);
        ...
        public List<string> GetAllUsernames()
        {
            ...
            {
                _sqLiteConnection.Open();
                var query = "SELECT DISTINCT username FROM caImages";
                var cmd = new SQLiteCommand(query, _sqLiteConnection);
                var result = cmd.ExecuteReader();
                while (result.Read())
                {
        ...
        public bool DeleteUser(string username)
        {
            ...
            {
                _sqLiteConnection.Open();
                var query = "DELETE FROM caImages WHERE username=@username";
                var cmd = new SQLiteCommand(query, _sqLiteConnection);
                cmd.Parameters.AddWithValue("username", username);
                var result = cmd.ExecuteNonQuery();
